...
function loadGroups(): FacebookGroup[] {
  try { return JSON.parse(readFileSync(GROUPS_FILE, "utf-8")) } catch { return [] }
}
...
export async function POST(request: NextRequest) {
  ...
  if (!url.includes("facebook.com/groups/")) {
    return NextResponse.json({ error: "Must be a Facebook group URL" }, { status: 400 })
  }
  const groups = loadGroups()
  const newGroup: FacebookGroup = { id: Date.now().toString(), name: name.trim(), url: url.trim() }
